...
// T-table 优化部分
static uint32_t T_table[4][256]; // 预计算的4个T-table
...
void init_T_table() {
    for (int i = 0; i < 256; i++) {
        uint32_t b = Sbox[i];
        // T0: L(Sbox[a])
        T_table[0][i] = b ^ ROTL32(b, 2) ^ ROTL32(b, 10) ^ ROTL32(b, 18) ^ ROTL32(b, 24);
        // T1: L(Sbox[a]) 的循环移位版本（用于密钥扩展）
        T_table[1][i] = b ^ ROTL32(b, 13) ^ ROTL32(b, 23);
        // T2/T3 可以留作其他优化（这里未使用）
    }
}

// 优化后的轮函数（使用T-table）
static uint32_t F_round_optimized(uint32_t X0, uint32_t X1, uint32_t X2, uint32_t X3, uint32_t rk) {
    uint32_t temp = X1 ^ X2 ^ X3 ^ rk;
    return X0 ^ (T_table[0][(temp >> 24) & 0xFF] ^
        T_table[0][(temp >> 16) & 0xFF] ^
        T_table[0][(temp >> 8) & 0xFF] ^
        T_table[0][temp & 0xFF]);
}
...
void SM4_KeySchedule(const uint8_t MK[16], uint32_t rk[32]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        // 使用预计算的T-table[1]（L'变换专用表）
        rk[i] = K[0] ^
            (T_table[1][(temp >> 24) & 0xFF] ^  // 字节3
                T_table[1][(temp >> 16) & 0xFF] ^  // 字节2
                T_table[1][(temp >> 8) & 0xFF] ^   // 字节1
                T_table[1][temp & 0xFF]);         // 字节0
...
void SM4_Encrypt(const uint8_t plaintext[16], const uint32_t rk[32], uint8_t ciphertext[16]) {
    ...
    // 32轮迭代
    for (i = 0; i < 32; i++) {
        uint32_t next_X = F_round_optimized(X[0], X[1], X[2], X[3], rk[i]);
        X[0] = X[1];
        X[1] = X[2];
        X[2] = X[3];
        X[3] = next_X;
...
void SM4_Decrypt(const uint8_t ciphertext[16], const uint32_t rk[32], uint8_t plaintext[16]) {
    ...
    // 32轮迭代（轮密钥逆序使用）
    for (i = 0; i < 32; i++) {
        uint32_t next_X = F_round_optimized(X[0], X[1], X[2], X[3], rk[31 - i]);
        X[0] = X[1];
        X[1] = X[2];
        X[2] = X[3];
        X[3] = next_X;
